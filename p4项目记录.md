...
主要的点就是考虑，作为一个新插入的 `tuple` ,他的 `MVCC` 版本链到底需不需要一个无效的 `ungolink`。同时由于测试用例都是单线程的，因此可以传入 `nullopt`，同时我为了保证 `undolog` 整体的语义，我传入了一个无效 `undolnk::nullopt` 作为初始插入的初始化。(完全没必要，因为可以在版本链查找时修改好，查询prev_version_.hasvalue() + is_valid()就行了...）
关于并发的问题，之后再说了。
#### 3.2 Commit
