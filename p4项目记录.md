...
在最后完成，我们再回到一开始，为什么在写`ReconstructTuple`可以跳过`is_deleted`的`tuple`了，主要还是在于：`is_deleted`只是一个标志，这一行对上层并不存在，因此版本链的返回可以跳过他去重组。再细究一点，就是`UndoLog`的实现方式了，在`txn_scan_test.cpp`内，带有`is_deleted`标志的log与普通的写log是交替出现的，他不会交叠，因此可以将整个UndoLog在语义上看作“tuple存在”和“列值变化”是分开标识的