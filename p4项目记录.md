...
以`update`为例，相较于之前的写法（仅我个人写法），会有一部分变动：
	...
	`MVCC`版本的`update`是一个`pipebreaker`，为了避免[Halloween](https://en.wikipedia.org/wiki/Halloween_Problem)类问题，简单来说就是避免`update`在不断更新自己已经更新过的内容。
按照这么分析，`delete`也是类似的，因为他也会修改`tuple`再更新`uodolog`。